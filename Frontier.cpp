...
int main(void) {
	...
	struct Pet pet;
	pet.sprite.setTexture(&tex.pet_f);
	pet.sprite.setSize(Vector2f(300, 300));
	pet.sprite.setPosition(W_WIDTH / 2 - 75, W_HEIGHT / 2 - 75);
...

	{
		...
		if (Keyboard::isKeyPressed(Keyboard::Left) && layer != 0 && !Keyboard::isKeyPressed(Keyboard::Down) && !Keyboard::isKeyPressed(Keyboard::Up)) { //대각선 이동 방지
			observing = 3;
			pet.sprite.setTexture(&tex.pet_l);
			pet.sprite.setPosition(player_pos.x+130, player_pos.y-70);
			...
			if (layer != 0) {
				pet.sprite.setPosition(player_pos.x + 135, player_pos.y - 70);
				switch (observing) {
				case 3:
					if (hand == 0)
		...
		if (Keyboard::isKeyPressed(Keyboard::Right) && layer != 0 && !Keyboard::isKeyPressed(Keyboard::Up) && !Keyboard::isKeyPressed(Keyboard::Down)) { //대각선 이동 방지
			observing = 4;
			pet.sprite.setTexture(&tex.pet_r);
			pet.sprite.setPosition(player_pos.x-140, player_pos.y-70);
		...
		if (Keyboard::isKeyPressed(Keyboard::Up) && layer != 0 && !Keyboard::isKeyPressed(Keyboard::Left) && !Keyboard::isKeyPressed(Keyboard::Right)) { //대각선 이동 방지
			observing = 2;
			pet.sprite.setTexture(&tex.pet_b);
			pet.sprite.setPosition(player_pos.x + 150, player_pos.y-90);
			window.draw(pet.sprite);
			...
			if (layer != 0) {
				pet.sprite.setPosition(player_pos.x + 150, player_pos.y - 95);
				switch (observing) {
				case 2:
					if (hand == 0)
		...
		if (Keyboard::isKeyPressed(Keyboard::Down) && layer != 0 && !Keyboard::isKeyPressed(Keyboard::Left) && !Keyboard::isKeyPressed(Keyboard::Right)) { //대각선 이동 방지
			...
			if (layer != 0) {
				pet.sprite.setPosition(player_pos.x + 140, player_pos.y - 67);
				switch (observing) {
				case 1:
					if (hand == 0)
						player.sprite.setTexture(&tex.main_ch_front_x);
